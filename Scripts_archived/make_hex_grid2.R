hex_grid_sfsample <- function(sf_tracks, cellsize, buffer) {
  ...
  hex_nb <- spdep::dnearneigh(hex_cent, cellsize - 0.01 * cellsize, cellsize + 0.01 * cellsize)

  #Pick back up from here
  visit <- unlist(unique(st_intersects(sf_tracks, hex_poly)))
  neighbor_df <- data.frame(hex_to = unlist(hex_nb),
  ...
  visit <- unlist(unique(st_intersects(sf_tracks, hex_poly)))
  neighbor_df <- data.frame(hex_from = rep(1:length(hex_cent), sapply(hex_nb, length)),
                            hex_to = unlist(hex_nb))
  neighbor_df$visit <- neighbor_df$hex_from %in% visit

  to_next <- st_coordinates(hex_cent[neighbor_df$hex_to]) -
    st_coordinates(hex_cent[neighbor_df$hex_from])
  neighbor_df$bearing_to_next <- (atan2(to_next[,1], to_next[,2]) * 180 / pi) %>%
    ifelse(. < 0, 360 + ., .)
  unit_xy <- cbind(cos(pi * (90 - neighbor_df$bearing_to_next) / 180),
                  sin(pi * (90 - neighbor_df$bearing_to_next) / 180))
